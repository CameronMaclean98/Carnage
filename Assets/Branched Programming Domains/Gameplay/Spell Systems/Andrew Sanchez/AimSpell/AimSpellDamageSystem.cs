...
public class AimSpellDamageSystem : JobComponentSystem
{
    ...
    struct DamageJob : IJobForEachWithEntity<Translation, Health>
    {
        ...
        public void Execute(Entity entity, int index, ref Translation position, ref Health health)
        {
            ...
            {
                if (math.distance(pos.Value, position.Value) < 30f)
                {
                    health.Value -= 0.4f;
                }
            }
    ...
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        EntityQuery m_Pos = GetEntityQuery(ComponentType.ReadOnly<Translation>(), ComponentType.ReadOnly<AimSpellComponent>());
